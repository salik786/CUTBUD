export default function Loading() {
  return (
    <main className="mx-auto flex w-full max-w-[520px] flex-1 flex-col px-6 py-8">
      ...
      <div className="mx-auto mt-5 h-8 w-2/3 animate-pulse rounded-lg bg-border" />
      <div className="mx-auto mt-2 h-4 w-3/4 animate-pulse rounded-lg bg-border" />
      <div className="mt-8 flex flex-col gap-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-16 animate-pulse rounded-xl bg-border" />
        ))}
      </div>
      <div className="mt-8 h-12 animate-pulse rounded-xl bg-border" />
    </main>
  );
}
